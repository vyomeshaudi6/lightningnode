...
func Open(dbPath string, modifiers ...OptionModifier) (*DB, error) {
	...
	options := &bbolt.Options{
        ...
        log.Infof("bbolt.open function")
	bdb, err := bbolt.Open(path, dbFilePermission, options)
	if err != nil {
		return nil, err
	}
        log.Infof("after bbolt.open function ")
	chanDB := &DB{
		DB:     bdb,
		dbPath: dbPath,
		now:    time.Now,
	...
	chanDB.graph = newChannelGraph(
        ...
        log.Infof("before chadb sync version")
	// Synchronize the version of database and apply migrations if needed.
	if err := chanDB.syncVersions(dbVersions); err != nil {
		bdb.Close()
		return nil, err
	}
        log.Infof("after  chadb sync version returning from here ")
	return chanDB, nil
}
